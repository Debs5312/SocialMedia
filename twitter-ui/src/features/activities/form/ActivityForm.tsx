...
export default observer(function ActvityForm() {
    ...
    function handleSubmit() {
        ...
        {
            activity.id = uuid();
            createActivity(activity).then(() => naviagte(`/activities/${activity.id}`))
        }
        else{
    ...
    return (
        <Segment clearing>
            <Form onSubmit={handleSubmit} autoComplete="off">
                ...
                <Button as={Link} to={`/activities/${activity.id}`} floated="right" negative type="button" content="Cancel" />
            </Form>
        </Segment>
    )
})